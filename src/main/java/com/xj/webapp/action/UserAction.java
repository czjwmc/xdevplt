...
import com.xj.webapp.util.RequestUtil;
...
public class UserAction extends BaseAction implements Preparable {
	...
	public String getQ() { // Added by wx Ver0.0.1.
		return query;
	}
	...
	public String delete() throws Exception {
		...
		try { // Modified by wx Ver0.0.1.
			...
			args.add(getText("userProfile.heading"));
	...
	public String edit() throws IOException {
		...
		if (id != null) {
			...
			user.setEnabled(true); // Added by wx Ver0.0.1.
		}
	...
	// Get related xdpGroups. add by wmc 2013/9/22
	public List<LabelValue> getRelatedXdpGroups() {
		List<LabelValue> relatedXdpGroups = new ArrayList<LabelValue>();
		Set<XdpGroup> xdpGroups = user.getXdpGroups();
		if (xdpGroups != null) {
			for (XdpGroup xdpGroup : xdpGroups) {
				...
				relatedXdpGroups.add(new LabelValue(label, xdpGroup.getId()
						.toString()));
			}
		}
		return relatedXdpGroups;
	}
	...
	public String save() throws Exception {
        ...
        // Check and set related xdpGroups.
        user.getXdpGroups().clear();
        String[] relatedXdpGroups = getRequest().getParameterValues("relatedXdpGroups");
		try {
			userManager.saveUser(user,relatedXdpGroups);
		...
		if (!"list".equals(from)) {
			...
			if (isNew) {
				...
				if (user.getEmail() != null && user.getEmail().length() > 0) { // Added
																				// by
																				// wx
																				// Ver0.0.1.
					...
					try {
						sendUserMessage(user,
								getText("newuser.email.message", args),
	 ...
	 */
	public String list() {
		if (query == null) { // Added by wx Ver0.0.1.
			query = (String) getSession().getAttribute("user_search_query");
		}
		...
		if (query != null) { // Added by wx Ver0.0.1.
			getSession().setAttribute("user_search_query", query);
		}
		return SUCCESS;
	}
