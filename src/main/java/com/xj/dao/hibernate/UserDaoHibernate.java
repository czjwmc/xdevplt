...
public class UserDaoHibernate extends GenericDaoHibernate<User, Long> implements UserDao, UserDetailsService {
    ...
    public List<User> getUsers() {
        ...
        Query qry = getSession().createQuery("from User u order by u.id desc)"); // Added by wx Ver0.0.1.
        return qry.list();
    }
    ...
    public List<User> getUsersEnabled() { // Added by wx Ver0.0.1.
        Query qry = getSession().createQuery("from User u where enabled=TRUE order by u.id desc)");
        return qry.list();
    }
    ...
    */
     ...
     * {@inheritDoc}
    */
    public String getUserPassword(Long userId) { // Added by wx Ver0.0.1 for allowing to change username.
    	JdbcTemplate jdbcTemplate =
                new JdbcTemplate(SessionFactoryUtils.getDataSource(getSessionFactory()));
        Table table = AnnotationUtils.findAnnotation(User.class, Table.class);
        return jdbcTemplate.queryForObject(
